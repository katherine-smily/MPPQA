...
if __name__ == '__main__':
    ...
    def preprocess(x):
        ...
        for i in range(args.shot):
            example_list.append({"question":train_examples['question'][i],
                                 "context":train_examples['context'][i],
                                 "answer":train_examples[i]['answers'][i]})
        example_prompt=f"\n Here are few examples:\n{json.dumps(example_list)}"
        x['prompt'] = ['Given a question and context, Answer the question using context, and seperate the answer spans with "、". Question: ' + xi[0] + ' Context: ' + xi[1] + example_prompt  
