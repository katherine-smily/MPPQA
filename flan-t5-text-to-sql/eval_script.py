...
def evaluate_mppqa(data_decoded):
    # predictions = [{'id': t[0], 'prediction_text':t[1].split("、")} for t in list(zip(data_decoded['id'], data_decoded['predicted_answers']))]
    
    # references = [{'id': t[0], 'answers':t[1]} for t in list(zip(data_decoded['id'], data_decoded['answers']))]
    results={}
    golds_answers={}
    result_score = None
    results_cp = {}
    for t in list(zip(data_decoded['id'], data_decoded['predicted_answers'])):
        results[t[0]] = t[1]
    for t in list(zip(data_decoded['id'], data_decoded['answers'])):
        golds_answers[t[0]] = t[1]

    keys = results.keys()
    for key in keys:
        results_cp[key] = [item for item in results[key]]

    if len(golds_answers) != 0:
