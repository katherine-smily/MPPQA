...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name",
                        default=r'/home/cike/bihan/projects/flan-t5-text-to-sql/opt/opt-350m',
    ...
    def preprocess(x):
        ...
        x["prompt"]=[]
        for xi in list(zip(x['question'],  x['context'])):
            content=[f"Given a question and context, Answer the question using context, and seperate the answer spans with '、'. ### Question: {xi[0]} ### Context: {xi[1]}"]
            truncated_content=tokenizer.decode(tokenizer(content,padding=True,truncation=True,max_length=3900,return_tensors="pt")["input_ids"][0])
            content=truncated_content+" ### Answer:"
            x['prompt'].append(content)
    ...
    def get_tokenized(x):
        ...
        x["input_ids"]=tokenizer(x['prompt'], padding=True,truncation=True,max_length=1900, return_tensors="pt")["input_ids"].tolist()
    ...
    def generate(x):
        ...
        x['output'] = [model.generate(torch.tensor(x_ids).reshape(1, -1).to(device), 
                            max_new_tokens=100,do_sample=True) for x_ids in x['input_ids']]
        return x
