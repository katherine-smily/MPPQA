...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name",
                        default=r'/home/cike/bihan/projects/llama2_mppqa/deepseek-llm-7b-chat', # meta-llama/Llama-2-7b-hf
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model_name,max_position_embeddings=1024,torch_dtype=torch.bfloat16,).cuda()
    ...
    def preprocess(x):
        x['prompt'] = [[{"role":"user",
                         "content":f"Given a question and context, Answer the question using context, and seperate the answer spans with '、'. ### Question: {xi[0]} ### Context: {xi[1]} ### Answer:"}]
    ...
    def get_tokenized(x):
        ...
        x["input_ids"]=[tokenizer.apply_chat_template(xi, add_generation_prompt=True,return_tensors="pt") for xi in x['prompt']]
        
        # x["input_ids"]=tokenizer.apply_chat_template(x['prompt'], add_generation_prompt=True,return_tensors="pt")
    ...
    def generate(x):
        ...
        x['output'] = [model.generate(torch.tensor(x_ids).to(device), 
    ...
    def decode(x):
        ...
        x['predicted_answers'] = [tokenizer.decode(torch.tensor(xo[0][x['input_ids'][i].shape[1]:]), skip_special_tokens=True, 
        clean_up_tokenization_spaces=False)[0].split("、") for i,xo in enumerate(x['output'])]
        return x
