...
class TaggerPlusForMultiSpanQA(BertPreTrainedModel):
    def __init__(self, config, structure_lambda, span_lambda):
        ...
        self.span_bilstm_classifier=nn.Sequential(nn.LSTM(input_size=self.H,hidden_size=self.H//2,bidirectional=True),nn.Linear(self.H, config.num_labels))
...
class ModelArguments:
    ...
    major_lambda: float= field(default=0.02)
    minor_lambda: float= field(default=0.02)
...
def main():
    ...
    def prepare_train_features(examples):
        ...
        for i, sample_index in enumerate(sample_mapping):
            ...
            # tokenized_examples["major_type"].append(structure_to_id[examples['major_type'][sample_index] if 'major_type' in examples else ''])
            # tokenized_examples["minor_type"].append(structure_to_id[examples['minor_type'][sample_index] if 'minor_type' in examples else ''])
