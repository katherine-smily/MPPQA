...
def predict_and_evaluate(model,dataloader,dataset,examples,golds):
    model.eval()
    model.sent_encode_model.eval()
    ...
    # The dictionaries we have to fill.sequence_ids
    ...
    # Evaluate on valid
    result_joint = multi_span_evaluate(all_predictions, golds)
    # Span adjustment
    ...
    # Evaluate again
    result_full = multi_span_evaluate(all_predictions, golds)
    return result_joint,result_full,all_predictions
# todo
class TaggerPlusForMultiSpanQA(BertPreTrainedModel):
    def __init__(self, config, span_lambda,args,sent_model):
        ...
        self.sent_encode_model=MultiGrainedAndSynticEnhancedModel(config, args).to(self.device_procedure)#.cuda()
        self.sent_encode_model.load_state_dict(sent_model.module.state_dict(),strict=False)
        ...
        self.bert=self.sent_encode_model.bert
        ...
        config.num_attention_heads=args.num_attention_heads # for span encoder
    ...
    def forward(
        ...
        if self.add_sent_tag_loss:
            ...
            for i,each_batch_num_nodes in enumerate(batch_num_nodes):
                ...
                each_sent_repr=torch.zeros(self.max_sent_num,sen_repr_gru_syn[begin_idx:begin_idx+each_batch_num_nodes].size(1),device=self.device_procedure) #.to(tmp_each_sent_repr)
                each_sent_repr[:min(sen_repr_gru_syn[begin_idx:begin_idx+each_batch_num_nodes].size(0),self.max_sent_num),:]=sen_repr_gru_syn[begin_idx:begin_idx+each_batch_num_nodes]
        ...
        for b in range(batch_size):
            mask=np.array(data["token_sent_idxs"][b])!=-1
            token_sent_matrix[b][mask]=x_sen_repr_gru_syn_att[b][data["token_sent_idxs"][b]][mask]
        data["token_sent_idxs"]=None
...
class DataTrainingArguments:
    ...
    continue_training_model_dir:str=field(
        default=""
    )
...
def main():
    ...
    if training_args.do_train:
        ...
        data_files = {'validation':os.path.join(data_args.data_dir, "valid.json")}
    ...
    def prepare_train_features(examples):
        ...
        features, conn_edges, edges, link_labels, link_type_labels, \
            sent_types, max_node_num_per_process,tokenize_sents,sent_tags, \
                tokenidx2sentenceidxs_list,edge_percent = \
            prepare_data(examples,filepath=args.data_dir,
                        max_seq_len=80,tokenizer=tokenizer,window=args.window_size,is_oversample=args.do_oversample_data,
                        graph_connection=args.graph_connection_type)
        ...
        tokenized_examples["token_sent_idxs"]=[]
        ...
        for i, sample_index in enumerate(sample_mapping):
            ...
            tokenized_examples["token_sent_idxs"].append(token_sent_idx)
    ...
    # for name,param in model.sent_encode_model.named_parameters():
    #     if 'bert' in name:
    ...
    if training_args.do_train and data_args.continue_training and data_args.continue_training_model_dir!="":
        previous_training_model_path=os.path.join(data_args.continue_training_model_dir, "tagger_plus_model.pt")
        # model.load_state_dict(torch.load(previous_training_model_path))
        checkpoint = torch.load(previous_training_model_path)
    ...
    if training_args.do_train:
        ...
        best_full_exact_f1=0.
        best_full_overlap_f1=0.
        best_joint_exact_f1=0.
        best_joint_overlap_f1=0.
        # Train!
        ...
        for epoch in range(int(training_args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if step % training_args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                    ...
                    completed_steps += 1
            print(f"evalute valid_dataset in epoch {epoch}......\n")
            valid_result_joint,valid_result_full,valid_all_predictions = \
                predict_and_evaluate(model,eval_dataloader,eval_dataset,eval_examples,golds)
            logger.info(f"***evaluating: valid_result_full:{valid_result_full}\n")
            if valid_result_joint["exact_match_f1"]>best_joint_exact_f1:
                best_joint_exact_f1=valid_result_joint["exact_match_f1"]
                # torch.save(model,best_joint_exact_f1_model_path)
                # print(f"***Saving best_joint_exact_f1 model, valid_result_joint:{valid_result_joint}\n")
                logger.info(f"***Saving best_joint_exact_f1 model, valid_result_joint:{valid_result_joint}\n")
            if valid_result_joint["overlap_f1"]>best_joint_overlap_f1:
                best_joint_overlap_f1=valid_result_joint["overlap_f1"]
                state={'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch,'lr_scheduler':lr_scheduler.state_dict()}
                if not os.path.exists(model_dir+"_best"):
                    os.makedirs(model_dir+"_best")
                best_model_path = os.path.join(model_dir+"_best", "tagger_plus_model.pt")
                torch.save(state,best_model_path)
                # torch.save(model,best_joint_overlap_f1_model_path)
                # print(f"***Saving best_joint_overlap_f1 model, valid_result_joint:{valid_result_joint}\n")
                logger.info(f"***Saving best_joint_overlap_f1 model, valid_result_joint:{valid_result_joint}\n")
            if valid_result_full["exact_match_f1"]>best_full_exact_f1:
                best_full_exact_f1=valid_result_full["exact_match_f1"]
                # torch.save(model,best_full_exact_f1_model_path)
                # print(f"***Saving best_full_exact_f1 model, valid_result_full:{valid_result_full}\n")
                logger.info(f"***Saving best_full_exact_f1 model, valid_result_full:{valid_result_full}\n")
            if valid_result_full["overlap_f1"]>best_full_overlap_f1:
                best_full_overlap_f1=valid_result_full["overlap_f1"]
                # torch.save(model,best_full_overlap_f1_model_path)
                # print(f"***Saving best_full_exact_f1 model, valid_result_full:{valid_result_full}\n")
                logger.info(f"***Saving best_full_exact_f1 model, valid_result_full:{valid_result_full}\n")

        # print best eval result:
        logger.info(f"Finish training!\n \
               best_joint_exact_f1:{best_joint_exact_f1},\n \
              best_joint_overlap_f1:{best_joint_overlap_f1},\n \
              best_full_exact_f1: {best_full_exact_f1},\n \
              best_full_overlap_f1: {best_full_overlap_f1}\n")
        # todo: 在valid集最高值时save model
        # save model
        # torch.save(model.state_dict(),model_path)
        state={'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch,'lr_scheduler':lr_scheduler.state_dict()}
        torch.save(state,model_path)
        logger.info(f"finish saving model!\n")
