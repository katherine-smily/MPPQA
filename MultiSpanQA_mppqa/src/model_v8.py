...
class AttentionBasedQAModel(torch.nn.Module):
    """
    correlation_func = 1, sij = x1^Tx2
    correlation_func = 2, sij = (Wx1)D(Wx2)
    correlation_func = 3, sij = Relu(Wx1)DRelu(Wx2) #缩放点积注意力函数
    ...
    def __init__(self, config, args,model):
        ...
        # 句子编码模型
        self.sent_encode_model=model

        # input_size和hidden_size的指定（后续训练的时候debug看一下跟什么有关）
        # todo
    ...
    def forward(self, data):
        ...
        # 2. 获取sentence编码
        # 3. 根据模型计算attention分数
        # 4. 根据attention分数获取前k句子
        # 5. 计算根据ground true与预测计算loss

        # 待想，groundtrue label怎么设置，数据格式怎么读取
