...
def get_sequence_outputs(label, sequence_outputs):
    prev_tag = 'O'
    begin_offset = 0
    chunks = []
    ...
    for i, chunk in enumerate(label + ['O']):
        ...
        tag = chunk[0]
        if end_of_chunk(prev_tag, tag):
            chunks.append(np.mean(sequence_outputs[begin_offset:i],axis=0))
        if start_of_chunk(prev_tag, tag):
            begin_offset = i
        prev_tag = tag
    return chunks  # k,768

def get_indexes(label,label2id):
    ...
    for i, chunk in enumerate(label + ['O']):
        ...
        tag = chunk[0]
        if end_of_chunk(prev_tag, tag):
            chunks.append(list(range(begin_offset,i)))
        if start_of_chunk(prev_tag, tag):
            begin_offset = i
        prev_tag = tag
    return chunks

def get_entities_2(label,token):
    indexes = [[]]  # 存放预测的多个spans的下标，
    flag=False
    label_list = ["B", "I", "O"]
    label2id = {l: i for i, l in enumerate(label_list)}
    for i in range(len(label)):
        if label[i] == label2id['B']: # B
            indexes.append([i])
            flag=True
        if label[i] == label2id['I'] and flag: # I
            indexes[-1].append(i)
        if label[i] == label2id['O']: # O
            flag=False
    s_span=[]
    if indexes==[[]]:
        return s_span
    for i,index in enumerate(indexes):
        s_span=s_span.append((' '.join(token[index[0]:index[-1]+1]), index[0],index[-1]+1))
        # s_span = token[index[0]:index[-1]+1,:]  # 输出每个span对应的embeddings
        # pred_spans[i] = s_span
        # span_mask[b,i] = 1
    return s_span
...
# tagger的preds与gold的评价指标计算，em（exact match）/ overlap： 
