...
# procedure model part
def set_seed_procedure(args):
    random.seed(args.seed_procedure)
    np.random.seed(args.seed_procedure)
    torch.manual_seed(args.seed_procedure)
    if args.n_gpu > 0:
        torch.cuda.manual_seed_all(args.seed_procedure)
...
class MultiGrainedAndSynticEnhancedModel(torch.nn.Module):
    def __init__(self, config, args):
        ...
        # # 句子节点表示
        # # gru
        # self.sen_repr_gru=None
        # # syn
        # self.sen_repr_syn=None
        # # gru+syn
        # self.sen_repr_gru_syn=None
        
        # # 边表示
        # self.sen_edge_repr=None

        # # 边关系
        # self.sen_link_probs=None

        # query与attention的模块，记得把query和doc的sentences单独拎出来
    ...
    # def forward(self, x, pos_ids, syn_graph_data,input_masks,segment_ids,senType,sen_lengths,num_nodes):
    def forward(self,data):
        # node_token_ids = data["x"]
        node_token_ids=[]
        for i in data["x"]:
            node_token_ids.extend(i)
        node_token_ids=torch.tensor(node_token_ids,device=self.device)
        # pos_ids = data["pos_ids"]
        pos_ids=[]
        for i in data["pos_ids"]:
            pos_ids.extend(i)
        pos_ids=torch.tensor(pos_ids,device=self.device)
        ...
        for each_edge_index in edge_index:
            num_nodes+=len(each_edge_index)
        # for each_syn_graph_data in syn_graph_data:
        #     num_nodes+=len(each_syn_graph_data)
        ...
        for triplet_graph_batch_idx in range(len(edge_index)):
            # for triplet_graph in triplet_graph_batch:
            for triplet_graph_idx in range(len(edge_index[triplet_graph_batch_idx])):
                # length = len(triplet_graph.entity.tolist())
                length = len(entity[triplet_graph_batch_idx][triplet_graph_idx])
                token_semantic_repr_tmp = pos_repr[idx][:length]
                syn_repr = self.rgcn_encoder(token_semantic_repr_tmp, torch.tensor(edge_index[triplet_graph_batch_idx][triplet_graph_idx],device=self.device),
                                             torch.tensor(edge_type[triplet_graph_batch_idx][triplet_graph_idx],device=self.device), torch.tensor(edge_norm[triplet_graph_batch_idx][triplet_graph_idx],device=self.device))
        ...
        # sen_repr_gru=dialog_level_node_repr.to(self.device)
        # # syn
        # sen_repr_syn=syn_graph_repr_att.to(self.device)
        # gru+syn
        sen_repr_gru_syn=dialog_level_senType_repr
        """
        ...
        # 节点node、连接link、关系relation的表示
        # todo: 增加question_mask
        # with snoop:
        reprs={
            # "sen_repr_gru":sen_repr_gru,
            # "sen_repr_syn":sen_repr_syn,
            "sen_repr_gru_syn":sen_repr_gru_syn,
            # "sen_edge_repr":sen_edge_repr,
            # "sen_link_probs":sen_link_probs
        }

        # return loss, sent_class_prob, y_sentType_label, link_probs, ylabels, link_type_probs, y_linkType_label, link_tags,reprs
        return loss,reprs
